...
const Word = ({ word, offset, isActive, offsets, containerHeight, containerWidth,keyboardHeight,animatedText,staticLineList}) => {
  ...
  const animatedStyle = useAnimatedStyle(() => {
    ...
    const shouldHide = offset.order.value===1 && interactiveSign.value==='+'&& offset.status.value!=='dragging'
 ...
 useAnimatedReaction(
  ()=>offset.order.value,
  (current,previous)=>{
    if (current===previous+1 && offset.status.value!=='dragging') {
 ...
 useAnimatedReaction(
  ...
  (current,previous)=>{
    if (current<previous && current===0 ) {
      ...
      originalOrder.value=offset.order.value
      const sign = offset.word.value[0] === '+' || offset.word.value[0] === '-' ? offset.word.value[0] : '+';
      interactiveSign.value=sign
    }
    if (current>previous && current===100) {
      ...
      const sign = offset.word.value[0] === '+' || offset.word.value[0] === '-' ? offset.word.value[0] : '+';
      interactiveSign.value=sign
      offset.width.value=withTiming(offset.order.value!==1 ? offset.width.value+27: interactiveSign.value==='+'?offset.width.value-5:offset.width.value+10,{},()=>{
        
        originalWidth.value=offset.width.value
        originalOrder.value=offset.order.value
        nothingValue.value = withTiming(2000,{duration:600},()=>{
    ...
    (current,previous)=>{
      if (current!==previous){
        ...
        if (current && !previous && !opExecute && !opFinished) {
          ...
          const sign = offset.word.value[0] === '+' || offset.word.value[0] === '-' ? offset.word.value[0] : '+';
          interactiveSign.value=sign
          offset.width.value=withTiming(offset.order.value!==1 ? offset.width.value-27:
             interactiveSign.value==='+'?offset.width.value+5:offset.width.value-10,{},()=>{
            //originalOrder.value=offset.order.value
    ...
    (current,previous)=>{
      if (current!==previous) {
        if (current===0 && current<previous && opExecute) {
          if (animatedText.value.length===0 && !opFinished) {
            ...
            if (!opFinished) {
              ...
              offset.word.value = animatedText.value.map(item => typeof item === 'object' ? `${item.numerator}/${item.denominator}` : item)
              .join('');
        ...
        if (previous===0 && current>previous && opFinished && offset.reInputted.value) {
          ...
          // Directly assign to animatedText.value using an IIFE
animatedText.value = (() => {
  const expression = offset.word.value;

  // Replace fractions with an object representation
  const modifiedExpression = expression.replace(/(\d+)\/(\d+)/g, (match, p1, p2) => {
      return `{numerator:${p1},denominator:${p2}}`;
  });

  // Split the string by operators and fractions
  const regex = /(\d+|\+|\-|\*|\/|\{numerator:\d+,denominator:\d+\})/g;
  const parts = modifiedExpression.match(regex);

  // Convert the fraction string back into an object
  return parts.map(part => {
      if (part.startsWith("{numerator")) {
          const match = part.match(/\{numerator:(\d+),denominator:(\d+)\}/);
