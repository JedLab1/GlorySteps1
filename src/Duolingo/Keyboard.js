...
export default function Keyboard({keyboardHeight,animatedText}) {
  ...
  const handleAddInput = (input) => {
    'worklet'
    const addedItem = input==='Frac'?{numerator:'2',denominator:'3'}:input
    animatedText.value=[...animatedText.value,addedItem]
  };
  const handleDelete = () => {
    'worklet'
    ...
    <Animated.View style={[styles.container,keyboardStyle]}>
      ...
      <View style={styles.keyboardGrid}>
          {characters.map((char, index) => (
            <TouchableOpacity
              key={index}
              style={[styles.key,{ height:containerHeight/(characters.length/5)-5,}]}
              onPress={()=>{
                if (char==='<') {
                  handleDelete()
                }else{
