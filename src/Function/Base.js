...
import { Canvas, Line, Text, Path, Skia, useFont,Circle, vec} from '@shopify/react-native-skia';
import { useAnimatedReaction, useSharedValue, withTiming } from 'react-native-reanimated';

// Define a list of strong colors
const strongColors = ['red'];
...
const Base = ({ 
  ...
  const arrowSize = 8;
  const coord = useSharedValue(vec(0,0))
  //{ x: origin+line.value * scale, y: 0 }
  ...
  useAnimatedReaction(
    ...
    (current,previous)=>{
      if (current!==previous && previous) {
        coord.value = withTiming(vec(origin+lines[1].value * scale,0))
      }
    }
  )

  //{ x: origin+line.value * scale, y: 0 }
  //{ x: origin+line.value * scale, y: axisLength }
  return (
      <Canvas style={{ width:axisLength+2, height: axisLength}}>
        ...
        {lines && lines.map((line,index)=> line.direction==='vertical' ?(
          <Line key={index} p1={line.p1} 
          p2={line.p2} color="#1B6FF5" strokeWidth={2} opacity={0.5}  />
        ):(
          <Line key={index} p1={line.p1} p2={line.p2} 
          color="#1B6FF5" strokeWidth={2} opacity={0.5} /> 
        )
        )}
